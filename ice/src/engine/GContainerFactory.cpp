...
void GContainerFactory::readFromOntology() {
	...
	typeMap.insert( { ontologyInterface->toShortIri(iri + "BooleanRep"),
			BasicRepresentationType::BOOL });
	typeMap.insert( { ontologyInterface->toShortIri(iri + "ByteRep"),
			BasicRepresentationType::BYTE });
	typeMap.insert( { ontologyInterface->toShortIri(iri + "UnsignedByteRep"),
			BasicRepresentationType::UNSIGNED_BYTE });
	typeMap.insert( { ontologyInterface->toShortIri(iri + "ShortRep"),
			BasicRepresentationType::SHORT });
	typeMap.insert( { ontologyInterface->toShortIri(iri + "IntegerRep"),
			BasicRepresentationType::INT });
	typeMap.insert( { ontologyInterface->toShortIri(iri + "LongRep"),
			BasicRepresentationType::LONG });
	typeMap.insert( { ontologyInterface->toShortIri(iri + "UnsignedShortRep"),
			BasicRepresentationType::UNSIGNED_SHORT });
	typeMap.insert( { ontologyInterface->toShortIri(iri + "UnsignedIntegerRep"),
			BasicRepresentationType::UNSIGNED_INT });
	typeMap.insert( { ontologyInterface->toShortIri(iri + "UnsignedLongRep"),
			BasicRepresentationType::UNSIGNED_LONG });
	typeMap.insert( { ontologyInterface->toShortIri(iri + "FloatRep"),
			BasicRepresentationType::FLOAT });
	typeMap.insert( { ontologyInterface->toShortIri(iri + "DoubleRep"),
			BasicRepresentationType::DOUBLE });
	typeMap.insert( { ontologyInterface->toShortIri(iri + "StringRep"),
			BasicRepresentationType::STRING });

	this->fromCSVStrings(std::move(csv));
}

std::shared_ptr<Representation> GContainerFactory::fromCSV(std::string reprStr,
		std::map<std::string, std::shared_ptr<Representation>> *tmpMap, const char delim) {
	if (reprStr.empty()) {
		return NULL;
	}
	if (!isprint(delim)) {
...
std::shared_ptr<Representation> GContainerFactory::addOrGet(std::string name,
...
GContainer* GContainerFactory::makeGContainerInstance(
		std::shared_ptr<Representation> representation) {
	if (representation->isBasic()) {
		...
		default:
			ins = nullptr;
			_log->error(
					"Unknown representation basic type '%v' for representation '%v'",
					rep->type, rep->name);
			break;
		}
...
std::shared_ptr<Transformation> GContainerFactory::fromXMLDesc(TransDesc* desc) {
	...
	if (false == rep) {
		_log->error(
				"Unknown target representation '%v' for transformation '%v', transformation can not be created",
	...
	if (inputCount <= 0) {
		_log->error(
				"Invalid number of inputs '%v' for transformation '%v', transformation can not be created",
				inputCount, desc->name);
	...
	if (scope == "") {
		_log->error(
				"Invalid scope '%v' for transformation '%v', transformation can not be created",
				desc->scope, desc->name);

		return std::shared_ptr<Transformation>();
	}

	std::shared_ptr<Transformation> trans = std::make_shared<Transformation>(
			this->shared_from_this(), desc->name, scope, rep);

// reading inputs
	std::map<int, std::shared_ptr<Representation>> inputs;

	for (auto input : desc->inputs) {
		auto inRep = this->getRepresentation(
				this->ontologyInterface->toShortIri(input.representation));

		if (false == rep) {
			_log->error(
					"Unknown input representation '%v' for transformation '%v', transformation can not be created",
...
bool GContainerFactory::extractOperations(std::shared_ptr<Transformation> transformation,
		std::shared_ptr<Representation> representation, std::vector<DimensionDesc> &ops,
		std::vector<std::string> &path,
	...
	for (auto operation : ops) {
		...
		if (false == repDim) {
			_log->error(
					"Unknown dimension '%v' for transformation '%v', transformation can not be created",
		...
		if (nullptr == pathTarget) {
			_log->error(
					"Unknown path to target dimension '%v' for transformation '%v', transformation can not be created",
		...
		case (XML_USE): {
			...
			if (false == repSource) {
				_log->error(
						"Unknown dimension source '%v' for transformation '%v', transformation can not be created",
			...
			for (int i = 0; i < pathDim->size(); ++i) {
				pathDim->at(i) = this->ontologyInterface->toShortIri(
						pathDim->at(i));
			}
		...
		case (XML_COMPLEX):
			result = this->extractOperations(transformation, repDim, operation.dims,
					path, reps);
			...
			break;
		default:
			_log->error("Unknown operation type '%v' in transformation '%v' from XML",
					operation.type, transformation->getName());
			return false;
			break;
		}
...
void GContainerFactory::printReps(std::shared_ptr<Representation> representation, int depth) {
	...
	for (int i = 0; i < representation->dimensions.size(); ++i) {
		...
		std::cout << std::string(depth + 1, ' ') << representation->dimensionNames.at(i)
				<< " " << rep->name << std::endl;
...
bool GContainerFactory::addTransformation(std::string &name,
...
std::shared_ptr<Transformation> GContainerFactory::getTransformation(std::string &sourceRep,
...
std::shared_ptr<Transformation> GContainerFactory::getTransformation(
		std::vector<std::string> &sourceReps, std::string &targetRep) {
	for (auto &trans : this->transformations) {
		if (trans.second->getTargetRepresentation()->name != targetRep)
			continue;
